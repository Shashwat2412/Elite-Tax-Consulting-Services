...
// Phone validation (10–15 digits, allows +, spaces, hyphens)
...
const phoneMatch = userInput.match(/\+?[0-9\s\-]{10,15}/)
if (phoneMatch) phone = phoneMatch[0]

if (lines.length >= 3) {
  name = lines[0].replace(/^(name:?|i'm|my name is)/i, "").trim()
  phone = phone || lines[1].replace(/^(phone:?|mobile:?)/i, "").trim()
  email = email || lines[2].replace(/^(email:?)/i, "").trim()
  requirement = lines.slice(3).join(" ").trim()
}
  ...
  botResponse = `⚠️ I need a bit more info:\n\n• A valid **phone number** (10–15 digits)\n• A valid **email address**\n\nExample:\nJohn Smith, +1 555-123-4567, john@email.com, Need help with business taxes`
}
